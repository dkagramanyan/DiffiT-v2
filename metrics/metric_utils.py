...
#
# Miscellaneous utilities for quality metrics.
...
def get_feature_detector_name(url):
    """Extract detector name from URL."""
    return os.path.splitext(url.split('/')[-1])[0]


def get_feature_detector(url, device=torch.device('cpu'), num_gpus=1, rank=0, verbose=False):
    """Load feature detector from URL with caching."""
    assert 0 <= rank < num_gpus
    key = (url, device)
...
class FeatureStats:
    """Accumulator for feature statistics (mean and covariance)."""
...
class ProgressMonitor:
    """Progress monitor for metric computation."""
...
def compute_feature_stats_for_dataset(
    ...
    **stats_kwargs
    ...
    """Compute feature statistics for real dataset images."""
    ...
    if opts.cache:
        ...
        cache_file = os.path.join('.', 'dnnlib', 'fid-cache', cache_tag + '.pkl')
    ...
    if opts.rank == 0:
        print('Computing feature statistics for dataset (first time, will be cached)...')
        if cache_file:
...
def compute_feature_stats_for_diffusion(
    opts, 
    detector_url, 
    detector_kwargs, 
    rel_lo=0, 
    ...
    **stats_kwargs
    ...
    """Compute feature statistics for generated images from diffusion model."""
