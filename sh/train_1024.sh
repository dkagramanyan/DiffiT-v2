#!/usr/bin/env bash
# DiffiT -- train at 1024x1024.
#
# Workstation:  bash sh/train_1024.sh               (detaches; output goes to logs/)
#               FOREGROUND=1 bash sh/train_1024.sh  (stays attached; output also goes to logs/)
...
# environment (DATA=<zip> GPUS=<n> bash sh/train_1024.sh). Anything after the script name
...
RUN_VARS=(CONDA_ENV OUTDIR CFG DATA GPUS BATCH_GPU KEEP_LAST AUGMENT NUM_FID_SAMPLES SEED
          WORKERS INIT_WEIGHTS CUDA_VISIBLE_DEVICES OMP_NUM_THREADS MKL_NUM_THREADS
