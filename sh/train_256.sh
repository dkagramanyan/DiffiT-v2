#!/usr/bin/env bash
# Train DiffiT at 256x256. Runs unmodified on a workstation
# (`bash sh/train_256.sh`) or a cluster
# (`sbatch --account=<proj> --partition=<part> --gpus=<N> sh/train_256.sh`).
...
diffit-train \
  ...
  --cfg diffit-256 \
  --data "${DATA:?set DATA=/path/to/256x256.zip}" \
  --gpus "${GPUS:-2}" \
