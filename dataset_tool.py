...
# Copyright (c) 2024, DiffiT authors.  Adapted for DiffiT.
#
...
"""Tool for creating ZIP/PNG based datasets for DiffiT training.

Usage examples:

    # Convert ImageNet to 64x64 resolution
    python dataset_tool.py --source=/path/to/imagenet/train --dest=./imagenet64.zip \\
        --transform=center-crop --resolution=64x64

    # Convert a folder of images to 256x256
    python dataset_tool.py --source=./my_images --dest=./dataset.zip \\
        --transform=center-crop --resolution=256x256

    # Create folder output (for debugging)
    python dataset_tool.py --source=./my_images --dest=./dataset/ \\
        --transform=center-crop --resolution=64x64

The output dataset can be used directly with train.py:
    
    python train.py --outdir=./runs --data=./imagenet64.zip --batch-gpu=32 --resolution=64
"""
