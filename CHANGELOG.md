...
- `download_models.sh` fetches the SD-VAEs into `$MODEL_CACHE/huggingface/hub`
  (`hf download --cache-dir`), so `MODEL_CACHE` now moves them too instead of
  leaving them in the default HuggingFace cache.
